...
export const ScoringPanel: React.FC<ScoringPanelProps> = ({ matchId, user }) => {
  ...
  const updateMatchScore = async (newRuns: number, newWickets: number, newOvers: number, newBalls: number, strikerId?: string, nsId?: string) => {
    const sId = strikerId || currentBatter?.id;
    const nId = nsId || nonStriker?.id;
    const scoreStr = `${newRuns}/${newWickets} (${newOvers}.${newBalls})${sId ? `|S:${sId}` : ''}${nId ? `|NS:${nId}` : ''}`;
    await supabase
      .from('matches')
      .update({ 
  ...
  const addBall = async (run: number, isExtra: boolean = false, extraType?: string) => {
    ...
    const { error: insertError } = await supabase.from('balls').insert([{
      ...
      over_number: nextOvers,
      ball_number: nextBalls,
      runs: run,
    ...
    await updateMatchScore(nextRuns, nextWickets, nextOvers, nextBalls, finalStriker?.id, finalNonStriker?.id);
  };
  ...
  const addWicket = async () => {
    if (wickets < 10) {
      ...
      await updateMatchScore(runs, nextWickets, nextOvers, nextBalls, currentBatter?.id, nonStriker?.id);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#282878] text-white p-8 rounded-[40px] shadow-2xl flex flex-col md:flex-row items-center justify-between relative overflow-hidden gap-8">
        {/* Decorative background elements */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -mr-32 -mt-32 blur-3xl" />
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-indigo-500/10 rounded-full -ml-24 -mb-24 blur-2xl" />
        
        {/* Left: Batters Stats */}
        <div className="relative z-10 flex flex-col gap-3 min-w-[180px] max-w-[220px] flex-shrink-0">
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col min-w-0">
              <span className="text-sm font-black text-white flex items-center gap-1 truncate">
                <span className="truncate">{currentBatter?.name || 'Batter'}</span>
                <span className="text-indigo-400 flex-shrink-0">*</span>
              </span>
              <span className="text-[9px] font-bold text-[#8e8ecf] uppercase tracking-tighter">
            ...
            <div className="text-right flex-shrink-0">
              <span className="text-2xl font-black text-white">{playerStats[currentBatter?.id]?.runs || 0}</span>
              <span className="text-xs font-bold text-[#8e8ecf] ml-1">({playerStats[currentBatter?.id]?.balls || 0})</span>
            </div>
          </div>
          <div className="h-[1px] w-full bg-white/10" />
          <div className="flex items-center justify-between gap-4 opacity-60">
            <div className="flex flex-col min-w-0">
              <span className="text-sm font-bold text-white/80 truncate">{nonStriker?.name || 'Batter'}</span>
              <span className="text-[9px] font-bold text-[#8e8ecf] uppercase tracking-tighter">
                SR: {playerStats[nonStriker?.id]?.balls > 0 ? ((playerStats[nonStriker?.id]?.runs / playerStats[nonStriker?.id]?.balls) * 100).toFixed(1) : '0.0'}
              </span>
            </div>
            <div className="text-right flex-shrink-0">
              <span className="text-xl font-bold text-white/80">{playerStats[nonStriker?.id]?.runs || 0}</span>
              <span className="text-xs font-bold text-[#8e8ecf] ml-1">({playerStats[nonStriker?.id]?.balls || 0})</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center text-center min-w-0">
          ...
          <div className="flex items-baseline gap-2">
            <h1 className="text-6xl font-black tracking-tighter leading-none">{runs}/{wickets}</h1>
            <span className="text-xl font-bold text-[#8e8ecf]">({overs}.{balls})</span>
          </div>
          {target !== null && (
          ...
          <div className="flex items-center gap-2 mt-2">
            <button 
              onClick={() => window.open(`/overlay/${matchId}`, '_blank')}
              className="flex items-center gap-1.5 px-3 py-1 bg-white/10 hover:bg-white/20 rounded-full text-[10px] font-bold text-white transition-all border border-white/5"
            >
            ...
            <button 
              onClick={() => {
                const url = `${window.location.origin}/overlay/${matchId}`;
                navigator.clipboard.writeText(url);
                setStatusMessage({ text: 'Overlay link copied to clipboard!', type: 'success' });
              }}
              className="flex items-center gap-1.5 px-3 py-1 bg-white/10 hover:bg-white/20 rounded-full text-[10px] font-bold text-white transition-all border border-white/5"
            >
              <Copy size={12} />
          ...
          <div className="flex gap-1.5 justify-center mt-4">
            {currentOverBalls.map((ball, i) => (
              <div 
                key={i} 
                className={cn(
                  "rounded-full flex items-center justify-center font-black border transition-all bg-white text-[#282878] border-white shadow-lg scale-110",
                  currentOverBalls.length > 8 ? "w-5 h-5 text-[8px]" : 
                  currentOverBalls.length > 6 ? "w-6 h-6 text-[9px]" : 
                  "w-7 h-7 text-[10px]"
                )}
              >
                {ball}
            ...
            {currentOverBalls.length < 6 && [...Array(6 - currentOverBalls.length)].map((_, i) => (
              <div 
                key={`empty-${i}`} 
                className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black border transition-all bg-white/5 text-white/20 border-white/10"
              />
            ))}
          </div>
        </div>
        ...
        {/* Right: Bowler Stats */}
        <div className="relative z-10 bg-white/5 backdrop-blur-md px-6 py-4 rounded-[32px] border border-white/10 min-w-[220px] flex-shrink-0">
          <p className="text-[10px] font-black text-[#8e8ecf] uppercase tracking-[0.3em] mb-2">BOWLING</p>
          
          <p className="text-xl font-black text-white truncate mb-1">{currentBowler?.name || 'Select Bowler'}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar">
              {[0, 1, 2, 3, 4, 6].map(run => (
                <button
                  key={run}
                  onClick={() => addBall(run)}
                  className="min-w-[60px] h-16 rounded-2xl bg-slate-50 hover:bg-indigo-600 hover:text-white transition-all font-black text-2xl text-slate-700 shadow-sm border border-slate-100 flex-shrink-0"
                >
                  {run}
                </button>
              ))}
              <button
                ...
                className="min-w-[60px] h-16 rounded-2xl bg-amber-50 text-amber-600 hover:bg-amber-600 hover:text-white transition-all font-black text-xl border border-amber-100 flex-shrink-0"
              >
                WD
              </button>
              <button
                onClick={() => addBall(0, true, 'No Ball')}
                className="min-w-[60px] h-16 rounded-2xl bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition-all font-black text-xl border border-rose-100 flex-shrink-0"
              >
                NB
              </button>
            </div>
          ...
          {/* Extras & Others */}
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-4">Extras & Others</h3>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              {updatingToss && <p className="text-[10px] text-indigo-600 font-bold animate-pulse">Updating toss info...</p>}
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-xs font-black">{match.current_innings || 1}</span>
              </div>
              <button
                onClick={() => {
                  setConfirmModal({
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              ...
              <button 
                ...
                className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md hover:bg-indigo-100 transition-colors flex items-center gap-1"
              >
                <RotateCcw size={10} />
                SWAP
              </button>
            ...
            <div className="space-y-4">
              <div 
                ...
                className="p-4 rounded-2xl bg-indigo-50 border border-indigo-100 cursor-pointer hover:bg-indigo-100 transition-colors group"
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <span className="font-black text-indigo-900">{currentBatter?.name || 'Select Batter'}*</span>
                ...
                <div className="flex gap-2">
                  <span className="text-[10px] font-bold text-indigo-400 uppercase">4s: {playerStats[currentBatter?.id]?.fours || 0}</span>
                  <span className="text-[10px] font-bold text-indigo-400 uppercase">6s: {playerStats[currentBatter?.id]?.sixes || 0}</span>
                  <span className="text-[10px] font-bold text-indigo-400 uppercase ml-auto">
                    SR: {playerStats[currentBatter?.id]?.balls > 0 ? ((playerStats[currentBatter?.id]?.runs / playerStats[currentBatter?.id]?.balls) * 100).toFixed(1) : '0.0'}
                  </span>
                </div>
              </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (showBatterSelect) {
                        ...
                        await updateMatchScore(runs, wickets, overs, balls, player.id, nonStriker?.id);
                      }
                      if (showNonStrikerSelect) {
                        setNonStriker(player);
                        await updateMatchScore(runs, wickets, overs, balls, currentBatter?.id, player.id);
                      }
                      if (showBowlerSelect) {
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={cn(
                  ...
                  confirmModal.type === 'danger' ? "bg-red-600 hover:bg-red-700" : "bg-indigo-600 hover:bg-indigo-700"
                )}
              >
                Confirm
              </button>
